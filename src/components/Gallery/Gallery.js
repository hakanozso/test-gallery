                ...
                <div className="gallery-outer" style={{ backgroundColor: 'black' }}>
                    ...
                    <div className="gallery-content" onTouchStart={() => {
                        ...
                        <button type="button" className="gallery-next" onClick={() => { handleImage(1) }}
                            onTouchStart={() => {
                                console.log('naber :>> ', "naber");
                                prev_next_show_attr("gallery-next")
                            }}
                            onTouchEnd={() => {
                                console.log('naber :>> ', "naber");
                                prev_next_hide_attr("gallery-next")
                            }}
                        >
                        ...
                        <div className="gallery-slider-content" onWheel={a}>
                            {imageLs?.map(image =>
                                image.selected == 1 ?
                                    (
                                        <div key={image.id}>
                                            <img src={image.url} alt="" onClick={(e) => handleImageSelect(e, image)} data-id={image.id} className="slider-image selected" style={{ backgroundImage: "url(" + image.url + ")" }} data-image-id="1" />
                                        </div>
                                    ) : (
                                        <div key={image.id}>
                                            <img src={image.url} alt="" onClick={(e) => handleImageSelect(e, image)} data-id={image.id} className="slider-image" style={{ backgroundImage: "url(" + image.url + ")" }} data-image-id="1" />
                                        </div>
                                    )
                            )}
                        </div>
